frappe.listview_settings['Sales Invoice'] = {
    refresh(listview) {
        ...
        listview.page.add_inner_button("Entity Inv to ZB", () => {
            frappe.call({
                method: 'pourtous.pourtous.doctype.zoho_books_api.zoho_books_api.fetch_unsynced_erp_entity_invoice_list',
                async: false,
                callback: (r) => {
                    if (r.message) {
                        const length = r.message.length;
                        console.log("Number of Entity invoices to sync: ", length);
                        console.log("Invoice List: ", r.message);
                        //console.log("r.message[0]['name']: ", r.message[0]["name"]);
                        let added = 0;
                        for (let i = 0; i < length; i++) {
                            setTimeout(() => {
                                frappe.call({
                                    method: 'pourtous.pourtous.doctype.zoho_books_api.zoho_books_api.sync_entity_inv_with_zoho_books',
                                    args: {
                                        invoice: r.message[i]["name"],
                                        customer: r.message[i]["customer"],
                                    },
                                ...
                                const count = i+1;
                                const message = "Adding "+count+" of "+length;
                                frappe.show_progress("Pushing Entity Invoices to Zoho Books", count, length, message);
                            }, 0);
                        }
                    }
                }
        ...
        listview.page.add_inner_button("Cards Inv to ZB", () => {
            frappe.call({
                method: 'pourtous.pourtous.doctype.zoho_books_api.zoho_books_api.fetch_unsynced_erp_card_invoice_list',
                async: false,
                callback: (r) => {
                    if (r.message) {
                        const length = r.message.length;
                        console.log("Number of Card invoices to sync: ", length);
                        console.log("Invoice List: ", r.message);
                        //console.log("r.message[0]['name']: ", r.message[0]["name"]);
                        let added = 0;
                        for (let i = 0; i < length; i++) {
                            setTimeout(() => {
                                frappe.call({
                                    method: 'pourtous.pourtous.doctype.zoho_books_api.zoho_books_api.sync_card_inv_with_zoho_books',
                                    args: {
                                        invoice: r.message[i]["name"],
                                    },
                                    async: false,
                                ...
                                const count = i+1;
                                const message = "Adding "+count+" of "+length;
                                frappe.show_progress("Pushing Card Invoices to Zoho Books", count, length, message);
                            }, 0);
                        }
                    }
                }
